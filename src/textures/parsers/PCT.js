...
/**
 * Decodes a Phaser Compact Texture Atlas (PCT) file from its raw text representation.
 *
 * This is a standalone helper function used by both the PCT loader and the PCT texture parser.
 * It converts the line-oriented PCT text format into a structured object containing page
 * definitions, folder dictionary, and fully-expanded frame data. See the Phaser Compact Texture
 * Atlas Format Specification for a full description of the format.
 *
 * @function Phaser.Textures.Parsers.PCT.decode
 * @since 4.0.0
 *
 * @param {string} text - The raw text contents of a PCT file.
 *
 * @return {?{pages: object[], folders: string[], frames: object}} The decoded PCT structure, or `null` if the file is invalid.
 */
 ...
 *
 * The `decoded` argument is the object produced by `PCT.decode` (or an equivalent, pre-decoded
 * structure). It must contain `pages`, `folders`, and `frames` fields. Frames are mapped to the
 * Texture's source images using the `page` index on each frame, which corresponds to the index of
 * the matching `TextureSource` on the Texture.
 *
 ...
 * @param {object} decoded - The decoded PCT data, as produced by `PCT.decode`.
 *
